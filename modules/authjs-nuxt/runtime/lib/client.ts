...
async function postToInternal({
  ...
  const response = await $fetch.raw<{ url: string }>(url, {
    ...
    // @ts-expect-error -- implicit boolean coercion in options.redirect
...
type ProviderID<P> = LiteralUnion<
  P extends RedirectableProviderType ? P | BuiltInProviderType
  : BuiltInProviderType
>;
...
>(
  ...
  authorizationParams?: SignInAuthorizationParams,
): Promise<undefined | SignInResponse> {
  ...
  try {
    ...
    const response = await postToInternal({
      url: _signInUrl,
      options,
      callbackUrl,
    });
...
export async function verifyClientSession() {
  ...
  try {
    if (cookies.value === null) throw new Error("No session found");
