...
export function getAuthJsSecret(options: AuthConfig) {
  ...
  if (!secret)
    throw new Error(
      "[authjs-nuxt] No secret found, please set a secret in your [...].ts handler or use environment variables",
    );
  return secret;
}
...
export function getServerOrigin(
  ...
  const origin =
    requestOrigin ?? serverOrigin.length > 0 ?
      serverOrigin
...
export function makeCookiesFromCookieString(
  cookieString: string | null | undefined,
) {
  if (!cookieString) return {};
  return Object.fromEntries(
    Object.entries(parse(cookieString)).filter(([k]) =>
      k.includes("next-auth"),
    ),
  );
}
...
export async function getRequestFromEvent(event: H3Event) {
  ...
  const body = event.method === "POST" ? await readRawBody(event) : undefined;
