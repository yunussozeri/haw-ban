<template>
  <div class="columntransition min-h-full flex-grow border-r border-white" @drop="onDrop($event)" @dragenter.prevent
    @dragover.prevent>
    <h2 class="columntransition__header mb-2 flex items-center justify-center rounded-t-md p-3 text-xl font-semibold"
      :style="{ backgroundColor: headerColor, color: headerTextColor }">
      {{ status }}
    </h2>
    <div class="columntransition__content min-h-[300px] rounded-b-md bg-gray-100 p-4">
...
const emit = defineEmits<{ (e: 'ticketMoved', ticketId: number, newStatus: string): void }>(); // Emit event

const onDrop = async (event: DragEvent) => {
  console.log("onDrop called");
  const ticketIdString = event.dataTransfer?.getData('ticketId');
  const originalStatus = event.dataTransfer?.getData('originalStatus') as Ticket['currentColumn'];  // Type assertion here

  // Convert ticketIdString to a number if it exists
  const ticketId = ticketIdString ? parseInt(ticketIdString, 10) : undefined;


  const ticketIndex = props.tickets.findIndex(ticket => ticket.id === ticketId); // Calculate once

  const updatedStatus = props.status === "Doing" ? "progress" : props.status.toLowerCase().replace(/\s+/g, '');
  ...
  try {
    ...
    props.tickets.splice(ticketIndex, 0, { ...props.tickets.find(t => t.id === ticketId), currentColumn: originalStatus });
